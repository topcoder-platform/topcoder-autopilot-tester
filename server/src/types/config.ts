...
export function normalizeFirst2FinishConfig(value: unknown): First2FinishConfig {
  const base = typeof value === 'object' && value !== null ? (value as Record<string, unknown>) : {};
  const fallback = DEFAULT_FIRST2FINISH_CONFIG;
...
export function normalizeAppConfig(raw: unknown): AppConfig {
  if (!raw || typeof raw !== 'object') {
    return { fullChallenge: DEFAULT_FLOW_CONFIG, first2finish: DEFAULT_FIRST2FINISH_CONFIG };
  }

  const data = raw as Record<string, unknown>;
  const hasNewStructure = Object.prototype.hasOwnProperty.call(data, 'fullChallenge') || Object.prototype.hasOwnProperty.call(data, 'first2finish');
